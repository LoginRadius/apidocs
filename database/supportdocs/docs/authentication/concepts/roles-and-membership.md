...
-  **Context:** Contexts in our APIs are essentially containers/data storage for custom roles and permissions. They are used to give additional permissions to your customer on a case by case basis (the Context). While Contexts are independent of the basic Roles and Permissions and are optional, Contexts can be useful when combined with Roles and Permissions.

For example, if you have multiple applications and you want that customer of one application should have some limited access to the other applications as well.

> **Note: Role, Permission** and **Context** are configured in the LoginRadius Identity Platform. The respective actions should be implemented on your application. 
> 
...
This guide will take you through the process to configure the following:
...
This section will take you through the configuration steps for managing the Roles and Permissions using APIs. If you have any flows that go beyond adding default Roles and Permissions to your customers you can leverage our Roles Management APIs as explained below:

- **Create Role:** You can create a role using the [Roles Create API.](/api/v2/user-registration/roles-create) This will add the role to the list of roles you have created. 

- **Role List:** You can get the list of roles using the [Roles List API.](/api/v2/user-registration/roles-list)
- **Assigned Role(s): **You can get the roles assigned to a particular user using the [Roles by UID API.](/api/v2/user-registration/roles-by-uid)
- **Unassign Role(s):** You can remove a role for a particular user using the [Unassign Role by UID.](/api/v2/user-registration/roles-unassign-to-user)
- **Add Permissions to Role:** You can add new permission(s) to an existing role using the [Account Add Permissions to Role API.](/api/v2/customer-identity-api/roles-management/add-permissions-to-role/)

For more details on Roles, Permission and Context APIs and their functioning, refer to [this document. ](/api/v2/customer-identity-api/roles-management/overview/)
...
- [LoginRadius APIs](#loginradiusapis6)
...
**Step 6:** Next, select a Roles for the Context from the drop-down list. Considering our scenario that role name is **Student**. Similarly, multiple roles can be involved in one context. 
...
**Step 7:** You can add the permission name in the Additional Permission(s) text box to assign additional permission(s) to the Online Classes context. This is an optional step. For example, the permission for the students with online classes contexts to access the classes 24/7.

**Step 8:** If desired, you can set up the expiration time for the context (Student Classes). This is an optional step.

The following screen displays the added information from Step 5 to 8:
...
The following chart displays the Online Classes context created for specific users to provide them additional permissions when compared to the other users falling into the default School context:
![Roles, Permissions and Context](https://apidocs.lrcontent.com/images/rpc11_102685e7f5b49255264.07713078.jpg "Roles, Permissions and Context")

The following is an example of the JSON code to implement the above-explained context:
...
- **Create Context:** You can create a context with a set of existing roles using the [Upsert Context API.](/api/v2/user-registration/roles-create-context)
- **Delete Context:** You can delete a context using the [Delete Context API.](/api/v2/customer-identity-api/roles-management/delete-context/)
- **Delete Role from Context:** You can delete a role from the context using the [Delete Role from Context API.](/api/v2/customer-identity-api/roles-management/delete-role-from-context/)
- **Delete Permissions from Context:** You can delete additional permission(s) from the context using the [Delete Permissions from Context API.](/api/v2/customer-identity-api/roles-management/delete-role-from-context/)
- **Get Context:** You can get the contexts that have been configured and the associated roles and permissions using the [Get Context API](/api/v2/customer-identity-api/roles-management/get-context/).
