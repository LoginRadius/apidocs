...
**Step 5:** In the **SAML App Name** field, enter an App name. If you have multiple SAML apps, the app name should be unique for each.

<div id="step6"></div>

**Step 6:** For Identity Provider Certificate and key, Generate LoginRadius' Certificate and Key: Self-signed certificate and key can be generated by using the following [document](/single-sign-on/concept/saml-miscellaneous/certificate/) or Using online tools, for example, with Bits and Digest Algorithm 2056, SHA256, respectively.

**Step 7:** Copy the above-generated values of LoginRadius' Key and certificate with headers and paste in the **ID Provider Certificate Key** and **ID Provider Certificate** fields, respectively.

**Step 8:** In the **Service Provider Certificate** field, enter the service provider's certificate.

**Step 9:** For **Attributes**, map the LoginRadius' fields with the Service Provider fields.
...
**Step 10:** For Name Id Format, select name Id format that is supported by the Service Provider. The default is `urn:oasis:names:tc:SAML:2.0:nameid-format:persistent`

**Step 11:** In **Login URL**, enter `https://<LoginRadius Site Name>.hub.loginradius.com/auth.aspx..`

**Step 12:** For **After Logout URL**, enter `https://<LoginRadius Site Name>.hub.loginradius.com/auth.aspx?action=logout`

**Step 13:** In the **Service Provider Logout URL**, enter the service provider logout URL (you will get the SLO URL from a third-party service provider). This Logout URL will be called in the Single Logout (SLO) SAML workflow.

**Step 14:** The **Default Request Binding**, select any of the binding values from the drop-down as per the service provider configuration, we support both types of binding values by default it will be urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST.

**Step 15:** In **Assertion Consumer Service Location**, enter location (you will get this from your service provider).

**Step 16:** In **Assertion Consumer Service Binding**, select the binding value as
...
**Step 17:** For **Relay State Parameter**, enter **RelayState**.

**Step 18:** For **App Audiences**, enter `https://<LoginRadius Site Name>.hub.loginradius.com/`. You can add more rows for App audiences to add multiple app audiences.

**Step 19:** Select **HTTPPost** from **SSO** Method.
